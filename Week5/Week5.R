...
genPredictions=function(stock){
  ...
  for (currday in c(trainstart:(trainend))) {
    ...
    to<-days[currday-1]
<<<<<<< HEAD
    # save(stock, "stock_a.rdata")
    train<-subset(stock,stock$date>=from&stock$date<=to)[3:ncol(stock)-1]
=======
...
>>>>>>> 8e6034a (changes)
    ...
    # Fit the Neural Net Model
    train_nn<-subset(stock,stock$date>=from&stock$date<=to)[3:25]
    test_nn<-subset(stock,stock$date==as.Date(days[currday]))
    
    x_train = data.frame(x_train, y_train)
    nn.fit <- neuralnet(nextreturn~.,data=x_train,hidden=c(5, 3),linear.output=TRUE)
    nn.predicted <- compute(nn.fit, test[3:24])
    rsq = R2_Score(nn.predicted$net.result, test$nextreturn)
    print(paste("RSQ:",rsq))
    preds = test
    preds$rsq<-rep(rsq,nrow(preds))
